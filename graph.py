...
def calculate_property(label):
    ...
    for char in split_to_char:
        for x in char:
           ...
           sum_properties = sum_properties + np.array(pick_key(x),dtype =  float)
...
def make_property_list(labels):
    property_list = []
    for label in labels:
        property_list = property_list+[calculate_property(label)]
    return property_list


model_3gram = "word2vec.model"
model = Word2Vec.load(model_3gram)
       
tsne = t3.BioTsne()
labels = model.wv.vocab.keys()

property_list = make_property_list(labels)

f = open("ngram_2D_vector.txt","rb")
